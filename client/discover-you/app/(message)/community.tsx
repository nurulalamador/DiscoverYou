...
export default function Community() {
    ...
    useEffect(() => {
        ...
        function loadMessages() {
            fetch(`${serverUrl}/community/inbox/${communityId}`, {
                method: "GET",
                credentials: "include"
            })
                .then(res => res.json())
    ...
    function sendMessage() {
        ...
        fetch(`${serverUrl}/community/send`, {
            method: "POST",
            credentials: "include",
            headers: {
                    ...
                    </TouchableOpacity>
                    {
                        ...
                        <>
                            <View style={styles.headerDetails}>
                                ...
                                <Text style={styles.communityActive}>{community.category}</Text>
                            </View>
                        </>
                    }
                </View>
                    ...
                    messages &&
                        ...
                        messages.map(function (msg: any, index: any) {
                            ...
                            return <View key={index}
                                style={msg.sent_by_me ? styles.ownMessageBoxContainer : styles.otherMessageBoxContainer}
                            >
                                {
                                    !msg.sent_by_me ?
                                    msg.sender_profile_picture_url ?
                                        <Image
                                            source={{ uri: serverUrl + msg.sender_profile_picture_url }}
                                            style={styles.profilePicture}
                                            contentFit="cover"
                                        /> :
                                        <View style={styles.pseudoProfilePicture}>
                                            <Text style={styles.pseudoProfilePictureText}>{msg.sender_name[0]}</Text>
                                        </View>
                                        :<></>
                                }
                                <View>
                                    { msg.sent_by_me ? <></> :
                                    <Text style={{ fontSize: 13, fontWeight: 500, marginHorizontal: 8 }}>{msg.sender_name}</Text>}
                                    <View
...
const styles = StyleSheet.create({
    ...
    ownMessageBoxContainer: {
        alignItems: 'flex-end',
        marginVertical: 4,
    },
    otherMessageBoxContainer: {
